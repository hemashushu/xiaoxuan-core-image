...
impl ReadOnlyDataSection<'_> {
    pub fn convert_to_entries(&self) -> Vec<ReadOnlyDataEntry> {
    ...
    pub fn convert_from_entries(entries: &[ReadOnlyDataEntry]) -> (Vec<DataItem>, Vec<u8>) {
...
mod tests {
    ...
    use crate::{
        common_sections::read_only_data_section::{DataItem, ReadOnlyDataSection},
        entry::ReadOnlyDataEntry,
    ...
    fn test_write_section() {
        let data_entry0 = ReadOnlyDataEntry::from_i32(11);
        let data_entry1 = ReadOnlyDataEntry::from_i64(13);
        let data_entry2 = ReadOnlyDataEntry::from_bytes(b"hello".to_vec(), 1);
        let data_entry3 = ReadOnlyDataEntry::from_f32(std::f32::consts::PI);
        let data_entry4 = ReadOnlyDataEntry::from_f64(std::f64::consts::E);
        let data_entry5 = ReadOnlyDataEntry::from_bytes(b"foo".to_vec(), 8);
        let data_entry6 = ReadOnlyDataEntry::from_i64(17);
        let data_entry7 = ReadOnlyDataEntry::from_i32(19);
        ...
        let (items, datas) = ReadOnlyDataSection::convert_from_entries(&entries);
        let section = ReadOnlyDataSection {
            items: &items,
            datas_data: &datas,
        };
    ...
    fn test_read_section() {
        ...
        let section = ReadOnlyDataSection::read(&section_data);
